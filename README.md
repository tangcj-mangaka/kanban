# 驴看板

一个个人用的看板应用，Windows 桌面端 + Android 手机端，数据在设备间实时互通。

和常见的看板不一样：**没有列**。每个看板是一块自由画布，卡片随意摆放拖动，分类完全靠标签。另有一个按标签自动分列的分组视图用来俯瞰。归档区叫**干草仓库**。

数据**本地优先**——每台设备存一份完整数据，离线照样能完整增删改查。同步只是有网时的后台行为，服务端不在线不影响任何设备使用。

## 下载安装

去 [Releases](https://github.com/tangcj-mangaka/kanban/releases) 拿最新版，按需要下载：

| 文件 | 装在哪 |
|---|---|
| `kanban-desktop-windows-*.zip` | Windows 电脑（客户端） |
| `kanban-server-windows-*.zip` | 其中**一台**常开的 Windows 电脑（同步服务端） |
| `kanban-android-*.apk` | 安卓手机 |

**不装服务端也能用**——那样就是一台设备上的单机看板，数据全在本地，功能一样不少。
想让电脑和手机互通，才需要找一台电脑跑服务端。

### Windows

两个 zip 解压后都是**整个文件夹**，别只把 exe 拖出来——旁边的 dll 少一个都跑不起来。

- 客户端：解压后直接运行 `kanban.exe`
- 服务端：运行 **`bin\server.exe`**（在 `bin` 子文件夹里，不在根目录），
  它会打印一个二维码和配对码

第一次运行 Windows 会弹「已保护你的电脑」，因为这个程序没买代码签名证书。
点「更多信息」→「仍要运行」。

### 安卓

APK 需要允许「安装未知来源的应用」。系统会在你点安装时引导你开这个开关。

### 让设备之间同步

1. 在那台跑服务端的电脑上开着 `server.exe`
2. 客户端里打开同步设置，扫二维码，或手填地址、端口和配对码
3. 配对码 5 分钟有效、只能用一次。要新的就在服务端电脑上打开 `http://localhost:8765`，点「换一个配对码」

**只在局域网内工作**，不走公网，数据不经过任何第三方服务器。

## 文档

| 文件 | 内容 |
|---|---|
| [docs/DESIGN.md](docs/DESIGN.md) | 完整设计文档：数据模型、同步机制、视图交互、UI 规范、开发路线 |
| [design/palette.html](design/palette.html) | 色板与图标方案对比页（浏览器打开） |

## 许可

[MIT](LICENSE)。随便用、随便改、随便分发，出了问题作者不负责。

## 开发环境

**在 Mac 上开发，Windows 的 `.exe` 由 GitHub Actions 云端构建。** 不需要任何 Windows 开发环境。

Flutter 无法在 macOS 上编译 Windows 桌面应用（依赖 MSVC 工具链，且不存在交叉编译方案）。但这只是**出包**的限制——P1 的全部内容都是平台无关的，在 macOS 桌面版上开发和验证即可。

### 日常开发（Mac）

```bash
cd packages/app && flutter run -d macos
```

```bash
cd packages/app && flutter analyze
```

### 拿 Windows 版

push 到 `main` 后，GitHub Actions 自动构建。到仓库的 **Actions** 页面下载 `kanban-windows` 产物，解压双击 `kanban.exe` 即可运行，目标机器不需要装任何东西。

也可以在 Actions 页面手动触发（`Run workflow`）。

### 已知的平台差异

在 macOS 上开发、最终跑在 Windows，以下地方需要在出包后实机确认一次：

- 中文默认字体不同（苹方 vs 微软雅黑），行高与字重观感有差异
- 滚动惯性、触控板手势
- 快捷键 `Cmd` / `Ctrl`
- 窗口标题栏样式、应用数据目录位置
- **P2 的托盘常驻和开机自启是 Windows 专属能力，Mac 上无法验证**，到那个阶段需要在 Windows 机器上实测

## 工程结构

```
kanban/
├── packages/
│   ├── shared/     # 数据模型 + 同步协议定义（客户端服务端共用）
│   ├── app/        # Flutter 客户端（Windows + Android）
│   └── server/     # Dart 服务端（局域网同步）
├── design/         # 设计资产
└── docs/           # 设计文档
```

## 进度

| 阶段 | 内容 | 状态 |
|---|---|---|
| P1 | 单机版：看板、画布、卡片、标签、分组视图、干草仓库。纯本地 SQLite | **完成** |
| P2 | 服务端 + 局域网实时同步 | **完成** |
| P3 | 附件系统 | **完成** |
| P4 | Android 客户端 | **完成** |
| P5 | 搜索、备份、UI 打磨 | — |

### P1 已实现

- **看板列表**：网格布局，新建 / 重命名 / 改封面色 / 删除（强确认）/ 拖动排序 / 按名称过滤
- **画布视图**：无限画布，卡片自由拖放、可调宽度、可折叠；滚轮平移、⌘+滚轮缩放、空格或中键拖拽平移；双击空白处建卡；一键整理成网格、适应全部
- **卡片**：标题、Markdown 正文（工具栏 + 预览）、颜色、多标签、评论、归档、删除
- **标签**：板内专属，可新建 / 重命名 / 改色 / 拖动排序 / 删除（只解除关联，卡片保留）
- **分组视图**：按全部标签自动分列 + 未分类，不可拖动，多标签卡片在每列各出现一份；可切换列内排序、隐藏空列
- **干草仓库**：可搜索、按标签筛选、单张捞回或彻底删除、清空全部（强确认）
- **底层**：op log 为真相源，字段级 LWW + 墓碑删除 + 小数序，为 P2 同步预留

### P2 已实现

- **服务端**：op log + 全局序号分配 + 广播中枢，不理解数据模型、从不物化
- **同步**：字段级 LWW、离线队列、自动重连、ACK 重放、编辑态提示
- **配对**：终端二维码 + 连接链接 + 6 位一次性配对码，三条路都留着
- **局域网自动发现**：UDP 广播，客户端设置页自动列出服务端
- **控制面板**：`http://localhost:8765`，看设备、备份、开关开机自启（仅本机可访问）
- **开机自启**：Windows 注册表 Run 项 + 隐藏窗口启动

启动服务端：

```bash
kanban-server.exe
```

### P3 已实现

- **按内容哈希存储**：同一张图在多张卡片里用只占一份磁盘，重复上传秒传
- **缩略图**：客户端生成（服务端不装图像库），长边 400px，小图不再生成
- **离线可用**：加附件只写本地，联网后自动补传；未传的标「待同步」
- **本地缓存**：上限 500MB，按最近最少使用淘汰，**绝不删还没传上去的**
- **断点续传**：下载支持 Range
- **延迟回收**：没人引用的文件先记成孤儿，30 天后才真删

### P4 已实现

- **Android 构建通了**，模拟器上跑通了完整的跨设备同步（Mac 推 250 条 → 手机全量拉下来）
- **清单配置**：正式版的 `INTERNET` 权限、明文流量（局域网服务端是 http/ws）
- **响应式**：窄屏顶栏只留图标、弹窗占满、搜索框自适应、工具条换行
- **手机默认进分组视图**：整屏一列、左右滑动切换，顶部列名标签条
- **触控手势**：画布双指捏合缩放、拖空白处平移（手机上没有滚轮和空格键）

构建 APK：

```bash
cd packages/app && flutter build apk --release
```

### 还没做的

- 全局搜索（跨看板搜卡片内容）—— P5
- 自动定时备份（现在是手动点一下）—— P5
- 真正的系统托盘图标（功能已由控制面板覆盖，见设计文档 §9.1）

### 需要在 Windows 实机验证的

Mac 上验证不了的部分，出包后逐项过一遍（设计文档 §8.4 有完整清单）：

- 开机自启：注册表项、隐藏窗口启动、路径带空格的情况
- 局域网广播：Windows 上受限广播才有效，与 macOS 行为不同
- 中文字体（微软雅黑）下的行高与字重观感
- 高 DPI 缩放下的画布表现

## 安卓客户端的签名

CI 会出一个 `kanban-android` APK。默认用 **debug 密钥**签名——能装能用，但
debug 密钥是每台构建机各自随机生成的，所以**每次 CI 出的包签名都不一样**，
装新版会被安卓以「签名不匹配」拒掉，得先卸载旧版（数据也就没了，除非已经
同步到服务器）。

要长期用就配一个固定密钥，只需做一次：

```bash
keytool -genkeypair -v -keystore kanban-release.jks -storetype PKCS12 \
  -keyalg RSA -keysize 2048 -validity 10000 -alias kanban
```

然后把 keystore 转成 base64。**用 `pbcopy` 直接进剪贴板，不要用鼠标从终端里
选中复制**——终端会按窗口宽度折行，选中时容易把折行和行尾空格一起带进去，
粘出来就不是合法的 base64 了（CI 会去掉空白再解码，但带进别的字符就没救了）：

```bash
base64 -i ~/kanban-release.jks > /tmp/ks.b64

# 自检：解回来必须和原文件一模一样
base64 -d -i /tmp/ks.b64 -o /tmp/ks-check.jks \
  && cmp ~/kanban-release.jks /tmp/ks-check.jks \
  && echo "OK，$(wc -c < /tmp/ks.b64) 字符" \
  || echo "有问题，别往下走"

pbcopy < /tmp/ks.b64
```

看到 `OK，xxxx 字符` 再往下。然后在 GitHub 仓库的
Settings → Secrets and variables → Actions 里加四个 secret：

| 名字 | 内容 |
|---|---|
| `ANDROID_KEYSTORE_BASE64` | 上一步的 base64 |
| `ANDROID_STORE_PASSWORD` | keystore 密码 |
| `ANDROID_KEY_PASSWORD` | key 密码。**PKCS12 格式下必须和上面一样**；留空的话 CI 会自动用 store 密码 |
| `ANDROID_KEY_ALIAS` | `kanban` |

配好之后 CI 自动改用正式签名，以后升级直接覆盖安装。

**keystore 文件和密码务必自己留好备份**：丢了就再也没法给已装的版本推更新，
只能卸载重装。仓库里不会有它——`.gitignore` 挡着 `key.properties` 和 `*.jks`。
